...
class ModelArguments(QuantizationArguments, ProcessorArguments, ExportArguments, VllmArguments):
    ...
    combination_type: Optional[str] = field(
        default=None,
        metadata={"help": "The merging type can be one of ['linear', 'ties', etc.]."},
    )
    majority_sign_method: Optional[str] = field(
        default="total",
        metadata={"help": "The method, should be one of ['total', 'frequency'], "
                          "to use to get the magnitude of the sign values. Should be used with [ties, ties_svd, dare_ties, dare_ties_svd]"},
    )
    combination_weights: Optional[List[float]] = field(
        default=None,
        metadata={"help": "List of float weights for merging adapters. Ex: 0.3 0.7 0.5 0.5 0.6 0.4"},
    )
    density: Optional[float] = field(
        default=None,
        metadata={"help": "Value between 0 and 1. 0 means all values are pruned and 1 means no values are pruned."},
    )
    merge_type: Optional[str] = field(
        default="default",
        metadata={"help": "Choose between 'default' and 'two_step' merging."}
    )
