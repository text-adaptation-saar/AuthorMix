    ...
    if model_args.adapter_name_or_path is not None:
        ...
        if len(adapter_to_merge) > 1 and model_args.combination_type is not None:
            if model_args.combination_weights is None:
                raise ValueError(
                    f"Combination_weights must be provided, if you use '{model_args.combination_type}' to merge lora adapters.")

            if model_args.merge_type == "two_step":
                if len(model_args.combination_weights) < len(adapter_to_merge):
                    raise ValueError(f"The number of combination_weights lower than number of adapters in two_step merge")
                model = two_step_merge(model, adapter_to_merge, model_args)
            else:
                if len(model_args.combination_weights) != len(adapter_to_merge):
                    raise ValueError(f"The number of combination_weights must be consistent with the number of adapters")
                model = simple_weighted_merge(model, adapter_to_merge, model_args)

            model = model.merge_and_unload()
            logger.info_rank0(f"Merged {len(adapter_to_merge)} adapter(s) into base model. (First, merged adapters with weights {model_args.combination_weights} "
                              f"and combination_type of {model_args.combination_type}, then merged into base model)")
...
def simple_weighted_merge(model, adapter_to_merge, model_args):
    weights = model_args.combination_weights
    adapter_names = []

    for idx, adapter in enumerate(adapter_to_merge):
        adapter_name = f"ad_{idx}"
        print(adapter_name)
        if idx == 0:
            model = PeftModelForCausalLM.from_pretrained(model, adapter, adapter_name=adapter_name)
        else:
            model.load_adapter(adapter, adapter_name)
        adapter_names.append(adapter_name)

    weighted_adapter_name = "merged_weighted_ad"
    print(f"adapters to merge: {adapter_names},\nweights: {weights},\ncombination_type:{model_args.combination_type},\nmerged_adapter_name:{weighted_adapter_name}")
    model.add_weighted_adapter(
        adapters=adapter_names,
        weights=weights,
        adapter_name=weighted_adapter_name,
        combination_type=model_args.combination_type,
        density=model_args.density,
        majority_sign_method=model_args.majority_sign_method
    )
...
def two_step_merge(model, adapter_to_merge, model_args):
    weights = model_args.combination_weights
    adapter_names = []
    ...
    # Intermediate merge 1: ad_0 + ad_1
    interm_1_name = "intermediate_merged_weighted_ad_1"
    model.add_weighted_adapter(
        adapters=[adapter_names[0], adapter_names[2]],
        weights=[weights[0], weights[1]],
        adapter_name=interm_1_name,
        combination_type=model_args.combination_type,
        density=model_args.density,
        majority_sign_method=model_args.majority_sign_method
    )
    model.set_adapter(interm_1_name)

    # Save intermediate 1
    os.makedirs(model_args.export_dir, exist_ok=True)
    save_path_1 = os.path.join(model_args.export_dir, interm_1_name)
    # model.save_adapter(save_path_1, interm_1_name)
    model.save_pretrained(save_path_1, selected_adapters=[interm_1_name])
    print(f"Saved {interm_1_name} to {save_path_1}")

    # Intermediate merge 2: ad_0 + ad_2
    interm_2_name = "intermediate_merged_weighted_ad_2"
    model.add_weighted_adapter(
        adapters=[adapter_names[1], adapter_names[2]],
        weights=[weights[2], weights[3]],
        adapter_name=interm_2_name,
        combination_type=model_args.combination_type,
        density=model_args.density,
        majority_sign_method=model_args.majority_sign_method
    )
    model.set_adapter(interm_2_name)

    # Save intermediate 2
    save_path_2 = os.path.join(model_args.export_dir, interm_2_name)
    # model.save_adapter(save_path_2, interm_2_name)
    model.save_pretrained(save_path_2, selected_adapters=[interm_2_name])
    print(f"Saved {interm_2_name} to {save_path_2}")

    # Clean up original adapters
    for name in adapter_names:
        model.delete_adapter(name)

    # Final merge: intermediate_1 + intermediate_2
    final_adapter_name = "final_merged_weighted_ad"
    model.add_weighted_adapter(
        adapters=[interm_1_name, interm_2_name],
    ...
    # Save final adapter
    save_path_final = os.path.join(model_args.export_dir, final_adapter_name)
    # model.save_adapter(save_path_final, final_adapter_name)
    model.save_pretrained(save_path_final, selected_adapters=[final_adapter_name])
    print(f"Saved final adapter: {final_adapter_name} to {save_path_final}")

    # Delete intermediate adapters from memory
    model.delete_adapter(interm_1_name)
    model.delete_adapter(interm_2_name)
