...
from llamafactory.x_my_pta_scripts.weight_optimize_hf.custom_lm.learnable_layerwise_weight_module_with_layer_wrapper import (
    LayerwiseLearnableWeightModule,
)
...
def visualize_layer_weights(model, output_path="layer_weights.png"):
    """Visualize per-layer weights as a heatmap"""
...
def _checkpoint_is_resumable(ckpt_dir: str) -> bool:
    """Return True if the checkpoint directory has a trainer_state.json (required to resume)."""
    return os.path.isfile(os.path.join(ckpt_dir, "trainer_state.json"))


def _find_latest_checkpoint(output_dir: str) -> Optional[str]:
    """Return path to checkpoint dir with the highest step that contains trainer_state.json.

    Walks from highest step downwards so that a truncated/incomplete final
    checkpoint (missing trainer_state.json) is transparently skipped in favour
    ...
    if not checkpoints:
        return None
    checkpoints.sort(key=lambda x: x[0], reverse=True)  # highest step first
    for step, ckpt_path in checkpoints:
        if _checkpoint_is_resumable(ckpt_path):
            return ckpt_path
        print(f"  [resume] Skipping checkpoint-{step}: trainer_state.json missing, trying previous checkpoint.")
    print("  [resume] No resumable checkpoint found (all missing trainer_state.json).")
    return None
...
def _load_optimized_weights_into_model(model, checkpoint_dir: str) -> bool:
    """Load per_layer_weights from checkpoint's optimized_weights.json into model.weight_logits. Returns True if loaded."""
    ...
    if not per_layer or not hasattr(model, "weight_logits"):
        return False
    adapter_names = list(model.adapter_names)
    num_layers = model.weight_logits.shape[0]
    num_adapters = model.weight_logits.shape[1]
    ...
    with torch.no_grad():
        model.weight_logits.copy_(torch.tensor(arr, dtype=model.weight_logits.dtype, device=model.weight_logits.device))
    return True
...
class WeightMonitorCallback(TrainerCallback):
    """Monitor per-layer weight evolution during GRPO training and save weights at checkpoints."""
        ...
        if model is not None and hasattr(model, 'get_all_weights'):
            ...
            for adapter_idx, adapter_name in enumerate(model.adapter_names):
                ...
                adapter_stats[adapter_name] = {
                    "mean": float(adapter_weights.mean()),
                    "std": float(adapter_weights.std()),
                    "min": float(adapter_weights.min()),
                    "max": float(adapter_weights.max()),
                }
            ...
            if logs:
                print(f"\n[Step {state.global_step}]")
                print(f"  Per-Adapter Statistics (across {weights.shape[0]} layers):")
...
def train_layerwise_weights_with_grpo_astrapop(
    base_model_path: str,
    ...
    # 1. Create model with learnable weights
    print("\nInitializing model with learnable weights...")
    model = LayerwiseLearnableWeightModule(
    ...
    print("\nFinal weights (all layers):")
    model.print_weights(verbose=True)

    visualize_layer_weights(model, os.path.join(output_dir, "layer_weights_final.png"))
    ...
    results = {
        ...
        'per_layer_weights': weight_detail['per_layer_weights'],
